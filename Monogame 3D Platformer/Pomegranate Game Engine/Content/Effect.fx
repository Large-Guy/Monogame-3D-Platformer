...
float4 MainPS(VertexShaderOutput input) : COLOR
{
	...
	brightness += CalculateSunBrightness(input, float3(0.5, -1, 0.5), 0.6);
	brightness += CalculatePointLightBrightness(input,float3(-15,1,0),1);
	float4 color = input.Color;
	float4 texColor = tex2D(MainTextureSampler, input.UV);
	color *= texColor;
	color *= float4(brightness, brightness, brightness, 1);
	return float4(floor(color.r * 32) / 32, floor(color.g * 32) / 32, floor(color.b * 32) / 32, texColor.a);
}
